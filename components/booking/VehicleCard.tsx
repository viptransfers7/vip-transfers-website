  ...
  return (
    <article className={`rounded-xl border bg-white p-3 transition ${selected ? "border-2 border-ink shadow-[0_10px_28px_rgba(10,10,11,.08)]" : "hairline hover:border-neutral-300"} ${disabled ? "opacity-55" : ""}`}>
      <div className="grid grid-cols-[72px_1fr] gap-3">
        <button type="button" onClick={onPreview} className="group relative h-[64px] overflow-hidden rounded-lg bg-neutral-100 text-left focus-ring" aria-label={`View ${vehicle.vehicleName} photo`}>
          <Image src={vehicle.imageUrl} alt={vehicle.vehicleName} fill className="object-cover transition duration-200 group-hover:scale-105" sizes="72px" />
        ...
        <button type="button" onClick={onSelect} disabled={disabled} className="grid min-w-0 grid-cols-[1fr_auto] gap-3 text-left">
          <span className="min-w-0">
            <span className="block truncate text-base font-black text-ink">{vehicle.vehicleName}</span>
            <span className="mt-0.5 block text-sm font-semibold text-neutral-400">{vehicle.category}</span>
          </span>
          <span className="text-right">
            <span className="block font-mono text-base font-semibold text-ink">{statusLabel}</span>
