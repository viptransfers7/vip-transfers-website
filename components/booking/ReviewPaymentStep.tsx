...
export function ReviewPaymentStep({
  ...
  return (
    <div>
      <h1 className="serif-title text-3xl md:text-4xl">Review and payment</h1>
      <div className="mt-5 grid gap-5 lg:grid-cols-[1fr_0.82fr]">
        <div className="grid gap-1 text-sm">
          {rows.map(([label, value]) => (
            <div key={label} className="grid gap-1 border-b hairline py-2.5 md:grid-cols-[160px_1fr]">
        ...
        <div className="grid content-start gap-4">
          ...
          <div className="border hairline bg-white p-4">
            <div className="text-sm font-black uppercase tracking-[0.14em] text-[#9a7b41]">{quote?.requiresCustomQuote ? "Quote request" : "Payment placeholder"}</div>
            ...
            <button type="button" onClick={onSubmit} disabled={submitting} className="btn btn-dark mt-5 w-full">
