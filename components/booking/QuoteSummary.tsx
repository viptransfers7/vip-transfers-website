...
export function QuoteSummary({ quote }: { quote: QuoteResponse | null }) {
  if (!quote) {
    return (
      <div className="rounded-xl border hairline bg-white p-4 text-sm text-neutral-600">
    ...
    <div className="rounded-xl border hairline bg-white p-4 md:p-5">
      <div className="flex items-start justify-between gap-4">
        <div>
          ...
          <div className="mt-1 font-mono text-3xl font-semibold md:text-4xl">${quote.finalPrice}</div>
        </div>
      ...
      <div className="mt-4 grid gap-2 text-sm">
        {quote.breakdown.map((item) => (
          <div key={item.label} className="flex justify-between border-t hairline pt-2">
            ...
            <span className="font-mono font-semibold">{item.amount < 0 ? "-" : ""}${Math.abs(item.amount)}</span>
          </div>
        ))}
      </div>
